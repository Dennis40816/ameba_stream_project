...
def update_camera_table(stop_event, update_interval=5):
    """
    Periodically updates and prints the camera table every `update_interval` seconds.

    Args:
        stop_event (threading.Event): Event to signal thread termination.
        update_interval (int, optional): Time interval between updates in seconds. Defaults to 5.
    """
    while not stop_event.is_set():
        with camera_lock:
            if connected_cameras:
                ...
                print("-" * 100)
                for ip, info in connected_cameras.items():
...
def main_server(host='0.0.0.0', port=12345):
    ...
    camera_table_thread = threading.Thread(target=update_camera_table, args=(stop_event,))
    camera_table_thread.daemon = True
    camera_table_thread.start()
